...
udpsink host=203.234.58.157 port=8001 sync=false";
	...
	if (!writer1.isOpened()) {
		cerr << "Writer open failed!" << endl; return -
			1;
	}
	Mat frame, gray;
	// dynamixel control
	...
	while (true)
	{
		...
		if (mx.kbhit())
		{
			...
			switch (c)
			{
			case 'x': goal1 = 0; goal2 = 0; break;
			case ' ': goal1 = 0; goal2 = 0; break;
			case 'w': goal1 = 100; goal2 = -100; break;
			case 's': goal1 = -100; goal2 = 100; break;
			case 'a': goal1 = -100; goal2 = -100; break;
			case 'd': goal1 = 100; goal2 = 100; break;
		...
		if (!mx.setVelocity(vel1, vel2)) {
			cout << "setVelocity error"
				<< endl; return -1;
		}
		if (ctrl_c_pressed) break;
		usleep(20 * 1000);
		gettimeofday(&end1, NULL);
		diff1 = end1.tv_sec + end1.tv_usec / 1000000.0 - start.tv_sec -
			start.tv_usec / 1000000.0;
		cout << "vel1:" << vel1 << ',' << "vel2:" << vel2 << ",time:" << diff1
			<< endl;
	}
	mx.close();
	return 0;
